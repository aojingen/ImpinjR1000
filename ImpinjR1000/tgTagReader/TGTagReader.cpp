...
struct ltstr
{
  ...
  {
    bool retval=(strcmp(l1,l2)<0);
    //printf("confronto: %s - %s -> l1<l2 :%i\n",l1,l2,retval);
    return retval;
  }
};

typedef std::map<char*, TGTag*, ltstr> TagMap;
typedef std::pair<char*, TGTag*> TagPair;
...
TagMap beepCache;
char tagsfile[80];
char prefix_include[12];
int rc,n,valtime,beeptime,enableSecurity;
int epcEncodingMode;
char pReaderHostName[20];
int progressivo=0;
time_t lastBeepT;
int epcSession;
int printDateTime=0;

char* hexStringToAsciiString(char* ascciCode, char buffer[])
{
        int hexlen=strlen(ascciCode)/2;
        char hex[5], *stop;
        memset(buffer,0,(sizeof(char)*(hexlen+1)));
        for(int i=0;i<hexlen;i++){

                hex[0] = '0';
                hex[1] = 'x';
                hex[2] = ascciCode[i*2];
                hex[3] = ascciCode[(i*2)+1];
                hex[4] = 0;

                buffer[i]=(char)strtol(hex, &stop, 16);
...
class ArmLlrpReader
{
  public:
    ...
    int
    beep (llrp_u1_t data, llrp_u16_t port  );
    //CGPOWriteData* gpoWriteData;
    CSET_READER_CONFIG* cmdCSET_READER_CONFIG;
  ...
  char *                        av[])
{
    ...
    system("echo \"OK\">STATUS");

    bzero(tagsfile,80);
    bzero(pReaderHostName,20);
    bzero(prefix_include,12);
    strcpy(prefix_include,"");
    epcSession=1;
    printDateTime=0;
    enableSecurity=0;
    ...
    static struct option loptions[] = {
        ...
        {"security",      no_argument,    0, 'u'},
        {"reader_addr",required_argument, 0, 'r'},
        {"tagsFile",     required_argument, 0, 'f'},
        {"val_time",   required_argument, 0, 't'},
        {"encoding",   required_argument, 0, 'e'},
        {"prefix_include", required_argument, 0, 'p'},
        {"epc_session", required_argument, 0, 's'},
        {"printDateTime", required_argument, 0, 'd'},
        {"beep_time",   required_argument, 0, 'b'}
    };
    while(1) {
        opt = getopt_long (ac, av, "vhqur:f:t:e:p:s:d:b:",
	...
	case 'q':
	    ...
	    break;
        case 'u':
            enableSecurity = strtol(optarg,NULL,10);
            break;
        case 'r':
            strcpy(pReaderHostName,optarg);
            break;
        case 'f':
            strcpy(tagsfile,optarg);
            break;
        case 'p':
            strcpy(prefix_include,optarg);
            break;
        case 't':
            valtime = strtol(optarg,NULL,10);
            break;
        case 'b':
            beeptime = strtol(optarg,NULL,10);
	    printf("beeptime impostato a: %d secondi",beeptime);
            break;
        case 'e':
            epcEncodingMode= strtol(optarg,NULL,10);
            break;
        case 's':
            epcSession= strtol(optarg,NULL,10);
            break;
        case 'd':
            printDateTime= strtol(optarg,NULL,10);
            break;
	default:
	    usage();
	    /* no return */
    ...
    //inizializzo la variabile dell'orario dell'ultimo beep
    time ( &lastBeepT );    
    ...
    if(pch != NULL){
        progressivo=atoi(pch)+1;
        printf ("Progressivo: %d\n",progressivo);
    }
...
void usage(void) {
    printf("Usage: ArmLlrpReader -r <reader_ip_address> -i <id_sys> -t <val_time> -e <encoding>\n"
    "\n"
    "Where:\n"
    "  -r, --reader_addr=ip address ip of the reader\n"
    "  -f, --tagsFile=tags file name into wich store data\n"
    "  -t, --val_time=secs          Tag validity seconds\n"
    "  -e, --encoding=0/1           Tag value rapresentation (0=raw/1=Ascii from Hex)\n"
    "  -b, --beep_time=secs         time in secons between one beep and other one (0=beep always a tag is detected)\n"
    "\n");
}
  ...
  char *                        pReaderHostName)
{
    ...
    //provo a collegarmi per 10 volte con uno sleep di 1 sec per ogni tentativo
    int ii=0;
    while(1) {
      ii++;
      ...
      if(0 != rc)
      {
	  printf("ERROR: connect: %s (%d) on attemps: %d\n", pConn->getConnectError(), rc,ii);
	  sleep(2);
      }
      else break;
    }
...
int
ArmLlrpReader::checkConnectionStatus (void)
{
    CMessage *                  pMessage;
...
int
ArmLlrpReader::scrubConfiguration (void)
{
...
int
ArmLlrpReader::resetConfigurationToFactoryDefaults (void)
{
...
int
ArmLlrpReader::deleteAllROSpecs (void)
{
    CDELETE_ROSPEC *            pCmd;
    CMessage *                  pRspMsg;
...
int
ArmLlrpReader::deleteAllAccessSpecs (void)
{
...
int
ArmLlrpReader::prepareReader(void)
{
...
int
ArmLlrpReader::getAllCapabilities (void)
{
...
int
ArmLlrpReader::getAllConfiguration (void)
{
...
int
ArmLlrpReader::configureNotificationStates (void)
{
 ...
 **             <DurationTrigger>604800000</DurationTrigger>
 **           </AISpecStopTrigger>
...
ArmLlrpReader::addROSpec (void)
{
    ...
    pAISpecStopTrigger->setDurationTrigger(4233600000); //49 giorni
#else
 ...
 TagObservationTriggerType_Upon_Seeing_No_More_New_Tags_For_Tms_Or_Timeout);
    ...
    pC1G2SingulationControl->setSession(epcSession);
    ...
    if(enableSecurity){
      CImpinjTagReportContentSelector * pImpinjTagReportContentSelector = new CImpinjTagReportContentSelector();
      CImpinjEnableSerializedTID* enableSerialTid=new CImpinjEnableSerializedTID();
      enableSerialTid->setSerializedTIDMode(ImpinjSerializedTIDMode_Enabled);
      pImpinjTagReportContentSelector->setImpinjEnableSerializedTID(enableSerialTid);
      
      pROReportSpec->addCustom(pImpinjTagReportContentSelector);
    }    
...
int
ArmLlrpReader::enableROSpec (void)
{
    CENABLE_ROSPEC *            pCmd;
    CMessage *                  pRspMsg;
 ...
 *****************************************************************************
 **
 ** @brief  Enable our ROSpec using IMPINJ_ENABLE_EXTENSIONS message
 **
 ** Enable the ROSpec that was added above.
 **
 ** The message we send is:
 **     <IMPINJ_ENABLE_EXTENSIONS MessageID='2002'>
 **           <ImpinjTagReportContentSelector>
 **             <ImpinjEnableSerializedTID>1</ImpinjEnableSerializedTID>
 **           </ImpinjTagReportContentSelector>
 **     </ENABLE_ROSPEC>
 **
 ** @return     ==0             Everything OK
...
int
ArmLlrpReader::enableImpinjExtensions (void)
{
    CIMPINJ_ENABLE_EXTENSIONS *            pExtCmd;
    CMessage *                  pRspMsg;
    CIMPINJ_ENABLE_EXTENSIONS_RESPONSE *   pRsp;

    pExtCmd = new CIMPINJ_ENABLE_EXTENSIONS();
    
    CCUSTOM_MESSAGE* pCmd;
  
    /*
     * Compose the command message
     */
    pCmd=new CCUSTOM_MESSAGE();
    pCmd->setVendorIdentifier(25882);
    pCmd->setMessageSubtype(21);
    
    llrp_bytesToEnd_t data=llrp_bytesToEnd_t(4);
    llrp_byte_t value[4];
    bzero(value,4);
    data.m_pValue=value;
    pCmd->setData(data);
     ...
     * Send the message, expect the response of certain type
     */
    int res=sendMessage(pCmd);
    printf("inviato messaggio custom con esito %d\n",res);

    res=sendMessage(pExtCmd);
    printf("inviato impinj extension enable con esito %d\n",res);
     ...
     * Check the LLRPStatus parameter.
     */
    //if(0 != checkLLRPStatus(pRsp->getLLRPStatus(), "enableImpinjExtensions"))
    //{
        /* checkLLRPStatus already tattled */
        //delete pRspMsg;
        //return -1;
    ...
    if(m_Verbose)
    {
        printf("INFO: ImpinjExtensions enabled\n");
    }
...
int
ArmLlrpReader::startAndMonitorOperation (void)
{
    CSTART_ROSPEC *             pCmd;
    int                         bDone = 0;
    int                         RetVal = 0;
    ...
    while(1)
    {
         ...
         */
        pMessage = recvMessage(864000000);
        if(NULL == pMessage)
        {
            /*
        ...
        {
            //system("echo \"WARNING: Ricevuto Messaggio da Non trattare\">>syslog");
        }
...
void
ArmLlrpReader::printTagReportData (
  CRO_ACCESS_REPORT *           pRO_ACCESS_REPORT)
{
     ...
     */
    /*for (
        Cur = pRO_ACCESS_REPORT->beginTagReporTGRENTtData();
...
void
ArmLlrpReader::printOneTagReportData (
  CTagReportData *              pTagReportData)
{
    const CTypeDescriptor *     pType;
    char                        aBuf[buffer_lenght];
    bzero(aBuf,buffer_lenght);
    ...
    else
    {
		...
		if(enableSecurity==1){
                ...
                char* mapKey=NULL;
                if(epcEncodingMode==1) {
                    char* decodedEpc=new char[buffer_lenght/2];
                    bzero(decodedEpc,buffer_lenght/2);
                    hexStringToAsciiString(buf,decodedEpc);
                    mapKey=decodedEpc;
                }
                else {
                    int epclen=strlen(buf);
                    mapKey=new char[epclen+1];
                    strcpy(mapKey,buf);
                    mapKey[epclen]='\0';
                }
                ...
                bool toWrite = false;
                bool existTag=true;
                TGTag* mappedTag=NULL;
                TagMap::iterator tagIter= tagCache.find(mapKey);
                if(tagIter==tagCache.end()) {
                    //printf(">>Nuovo tag:%s\n",decodedEpc);
                    toWrite=true;
                    existTag=false;
                }
                else {
                    mappedTag=tagIter->second;
                    time_t lastRilevT=mappedTag->tempo;
                    //printf("corrente %s- ultimo %s: %d - %d\n", decodedEpc, mappedTag->epc,tempo.Seconds, lastRilevT.Seconds);
                    int diff=tagrawtime-lastRilevT;
                    //printf("Chip %li esistente differenza tempo:%d\n",longEpc,diff);
                    if(diff>valtime){
                        //printf("Tempo validita %s scaduto, riscrivo il chip\n",decodedEpc);
                        toWrite=true;
                    }
                }
                ...
                if (toWrite)
                {
                    //rimetto nella map una copia dell'ultima istanza di tag
                    TGTag* toMapTag=NULL;

                    if(!existTag){
                      //toMapTag=(TGTag*)malloc(sizeof(TGTag));
                      toMapTag=new TGTag();
                      toMapTag->epc=mapKey;
                      tagCache.insert(TagPair(mapKey,toMapTag));
                    ...
                    //printf("Scritto valore epc nel nuovo TGTag\n");
                    toMapTag->tempo=tagrawtime;
                    //printf("Assegnato tempo al nuovo TGTag\n");


                    timeval curTime;
                    gettimeofday(&curTime, NULL);
                    int milli = (pTagReportData->getLastSeenTimestampUTC()->getMicroseconds()-((pTagReportData->getLastSeenTimestampUTC()->getMicroseconds()/1000000)*1000000))/1000;
                    ...
                    rawtime=pTagReportData->getLastSeenTimestampUTC()->getMicroseconds()/1000000;
                    timeinfo = localtime(&rawtime);

		    if(printDateTime==0){
		      strftime(tmsBuffer,9,"%H:%M:%S",timeinfo);
		    }
		    else {
		      strftime(tmsBuffer,21,"%Y-%m-%d-%H:%M:%S",timeinfo);
		    }

                    char currentTime[90] = "";
                    sprintf(currentTime, "%s.%03d", tmsBuffer, milli);
		    ...
		    if(NULL != pTagReportData->getAntennaID())
		    {
                      ...
                      sprintf(lineBuffer,"%s;%s;%d;%d\n",toMapTag->epc,currentTime,pTagReportData->getAntennaID()->getAntennaID(),progressivo);

		    }
		    else
		    {
                      sprintf(lineBuffer,"%s;%s;0;%d\n",toMapTag->epc,currentTime,progressivo);
		    }
		    progressivo=progressivo+1;
		    

                    fp = fopen(tagsfile,"a");
                    fprintf(fp,lineBuffer);
                    fclose(fp);

                    /*
                     * Gestione Beep: genero un beep solo se nella mappa con chiava la sottostringa composta
                     * dell'epc del tag meno i primi tre caratteri non è presente tale chiave oppure
                     * se presente con un tempo di rilevamento inferiore ai 2 secondi
                     */

                    bool toBeep=true;
		    if(beeptime!=0){
		      int diff=rawtime-lastBeepT;
		      if(diff<beeptime){
			  toBeep=false;
		      }
		      else {
		    ...
		    if(toBeep && prefix_include!=NULL && strcmp(prefix_include,"")!=0){
			char* foundPrefToInc=strstr(mapKey,prefix_include);
			if(foundPrefToInc!=NULL){
			    int dposfound = int (foundPrefToInc - mapKey);
			    if(dposfound==0)
			      toBeep=true;
			    else
			      toBeep=false;
...
int
ArmLlrpReader::beep (llrp_u1_t data, llrp_u16_t port  )
{
    //system("echo \"Istanzio CSET_READER_CONFIG\">>log");
    /*
     * Compose the command message
     */

    if(cmdCSET_READER_CONFIG==NULL)
        cmdCSET_READER_CONFIG = new CSET_READER_CONFIG();
    cmdCSET_READER_CONFIG->setMessageID(111);
    cmdCSET_READER_CONFIG->setResetToFactoryDefault(0);

    //system("echo \"faccio il clear del GPOWriteData in cmdCSET_READER_CONFIG\">>syslog");
    cmdCSET_READER_CONFIG->clearGPOWriteData();

    //system("echo \"clear effettuato. Ora aggiungo i nuovi gpoWriteData\">>syslog");
    CGPOWriteData* gpoWriteData=new CGPOWriteData();
    gpoWriteData->setGPOPortNumber(port);
    gpoWriteData->setGPOData(data);
    cmdCSET_READER_CONFIG->addGPOWriteData (gpoWriteData);
    //system("echo \"Nuovi gpoWriteData aggiunti\">>syslog");

    /*
     * Send the message, expect the response of certain type
     */
    sendMessage(cmdCSET_READER_CONFIG);
    //system("echo \"sendMessage eseguito\">>log");
